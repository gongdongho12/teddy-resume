...
Before changing an established resume layout, show the exact visual behavior that would change and wait for explicit approval. This includes column counts, content visibility, section order, forced page boundaries, card grouping, and density or typography changes made to reduce page count. A direct request authorizes only the named layout change; do not broaden it into adjacent restructuring. Keep Other Activities in the existing two-column desktop/print layout unless the user explicitly approves a different structure.
